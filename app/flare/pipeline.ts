...
const FLARE_COLOR = [18 / 255, 1, 128 / 255] as const;
...
export class FlarePipeline {
  ...
  constructor(private readonly gpu: Gpu, private readonly output: Target) {
    ...
    try {
      this.effects = {
        logo: effect(gpu, fullscreen(logoWgsl), { label: "safe-flare-logo" }),
        rim: effect(gpu, fullscreen(rimWgsl), { label: "safe-flare-rim" }),
        rimBlurH: effect(gpu, fullscreen(blurWgsl), {
          label: "safe-flare-rim-horizontal",
        }),
        rimBlurV: effect(gpu, fullscreen(blurWgsl), {
          label: "safe-flare-rim-vertical",
        }),
        composite: effect(gpu, fullscreen(compositeWgsl), {
          label: "safe-flare-composite",
        }),
      };
    } catch (error) {
